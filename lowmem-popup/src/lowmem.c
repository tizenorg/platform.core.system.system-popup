...
int lowmem_create_and_show_basic_popup(struct appdata *ad)
{
	...
	elm_popup_timeout_set(ad->popup, 3);
...
static void
main_quit_cb(void *data, Evas_Object *obj, const char *emission,
	     const char *source)
{
	elm_exit();
}
...
int lowmem_start(void *data)
{
	...
	ret_val = lowmem_create_indicator(ad);
	if (ret_val != 0)
		return -1;
